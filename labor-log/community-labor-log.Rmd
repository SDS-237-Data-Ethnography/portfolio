...
my_labor <- tibble(Labor = character(), Date = character()) %>%
  ...
  add_row(Labor = "[Class Notes 1]()", Date = NA) %>%
  add_row(Labor = "[Class Notes 2]()", Date = NA) %>%
  add_row(Labor = "[Class Notes 3]()", Date = NA) %>%
  add_row(Labor = "[Class Notes 4]()", Date = NA) %>%
  ...
  mutate(Points = case_when(grepl("Slack", Labor) & grepl("slack.com", Labor) & !is.na(Date)~ 1,
                            grepl("Class", Labor) & grepl("slack.com", Labor) & !is.na(Date)~ 2,
                            Labor == "Group Evaluations" & !is.na(Date) ~ 1,
...
kable(my_labor) %>%
  ...
  row_spec(nrow(my_labor), bold = T, color = "white", background = "#D7261E") ##highlight the total row
```
